...
// ─── GET /api/session/status ──────────────────────────────────
// Monitorado pelo Frontend (Polling)
app.get("/api/session/status", async (_req, res) => {
    try {
        const { data } = await supabase
            .from("app_config")
...
// ─── POST /api/session/reconnect ──────────────────────────────
// Rota acionada pelo botão do Frontend
app.post("/api/session/reconnect", async (req, res) => {
    try {
        const { email, password } = req.body;
        ...
        if (!ONEDRIVE_READER_URL) {
            return res.status(500).json({ error: "ONEDRIVE_READER_URL não configurada no ambiente" });
        }

        // Repassa a solicitação para o serviço ONEDRIVE-READER (isolado no Render)
        // Aumentado timeout para 30s devido ao cold start do Render Free
        const response = await fetch(`${ONEDRIVE_READER_URL}/internal/start-login`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ email, password, secret: INTERNAL_SECRET }),
        ...
        if (!response.ok) {
            ...
            return res.status(response.status).json({ 
                error: errData.error || "O serviço de automação retornou um erro." 
            });
        }

        res.json({ success: true, message: "Processo de login iniciado com sucesso" });

    } catch (err: any) {
        console.error("[Reconnect Error]:", err.message);
        res.status(500).json({ error: "Falha na comunicação com o serviço de automação." });
    }
});
