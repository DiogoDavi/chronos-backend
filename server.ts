...
app.use(cors({ origin: '*', methods: ["GET", "POST"] })); // Liberação total para teste
app.use(express.json());
...
app.get("/api/premises", async (_req, res) => {
  try {
    ...
    // Organiza por unidade para facilitar a vida do Frontend
    const formatted = data.reduce((acc: any, item: any) => {
      acc[item.unit] = item;
      return acc;
    }, {});
...
app.get("/", (req, res) => res.send("🚀 API Chronos Ativa"));
